...
use crate::models::{User, FileTransfer, TransferStatus};
...
impl FileTransferManager {
    ...
    pub fn start_file_transfer_service(&mut self) -> AppResult<()> {
        ...
        let listener = TcpListener::bind(format!("0.0.0.0:{}", FILE_TRANSFER_PORT))
            .map_err(|e| AppError::NetworkError(format!("Failed to bind to port {}: {}", FILE_TRANSFER_PORT, e)))?;
            ...
            loop {
                ...
                match listener.accept() {
                    Ok((stream, addr)) => {
                        ...
                        tokio::spawn(async move {
                            if let Err(e) = handle_file_connection(stream, addr, transfers_clone, connections_clone, local_user_clone).await {
    ...
    pub async fn send_file(&mut self, _peer_id: &str, _file_path: &str) -> AppResult<FileTransfer> {
        ...
        Err(AppError::FileTransferError("Use send_file_with_peer method instead".to_string()))
    }
    
    pub async fn send_file_with_peer(&mut self, peer_id: &str, file_path: &str, peer_ip: &str) -> AppResult<FileTransfer> {
    ...
    async fn send_transfer_request(&self, _peer_id: &str, transfer: &FileTransfer) -> AppResult<()> {
        ...
        stream.write_all(transfer_json.as_bytes()).await
    ...
    pub async fn accept_transfer(&mut self, transfer_id: &str, save_path: &str) -> AppResult<()> {
        ...
        let transfer = {
            ...
            let transfer = transfers.get_mut(transfer_id).ok_or_else(|| {
                AppError::TransferNotFound(transfer_id.to_string())
            })?;
...
async fn send_file_data(
    ...
    let addr = format!("{}:{}", recipient_ip, FILE_TRANSFER_PORT);
    let mut stream = AsyncTcpStream::connect(addr).await.map_err(|e| {
        AppError::NetworkError(format!("Failed to connect to recipient: {}", e))
    })?;
    
    // Send transfer ID
    stream.write_all(transfer.id.as_bytes()).await.map_err(|e| {
        AppError::NetworkError(format!("Failed to send transfer ID: {}", e))
    })?;
...
async fn receive_file_data(
    ...
    let mut stream = AsyncTcpStream::connect(addr).await.map_err(|e| {
        AppError::NetworkError(format!("Failed to connect to sender: {}", e))
    })?;
...
async fn handle_file_connection(
    ...
    let transfer = {
        ...
        transfers.get(&transfer_id).cloned().ok_or_else(|| {
            AppError::TransferNotFound(transfer_id.clone())
        })?
    };
